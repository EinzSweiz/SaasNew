        ...
        if not self.stripe_id:
            if self.init_email_confirmed and self.init_email:
                ...
                if email != '' or email is not None:
                    stripe_id = create_customer(metadata={'user_id': self.user.id,
...
def allauth_user_signed_handler(request, user, *args, **kwargs):
    email = user.email
...
allauth_user_signed_up.connect(allauth_user_signed_handler)
